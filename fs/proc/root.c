		...
		size_t count, loff_t *ppos)
{
	...
	char deferred_write_string[5] = {'\0'};

	if (!deferred_initcalls_done) {

		if (count > (sizeof(deferred_write_string) - 1))
		return -EINVAL;

		if (copy_from_user(deferred_write_string, buf, count))
		return -EFAULT;

		deferred_write_string[count] = '\0';

		mask = simple_strtoul(deferred_write_string, NULL, 0);

		if((mask >= 0) && (mask <= 15)) {
			do_deferred_initcalls(mask);
